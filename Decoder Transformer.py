...
class Preprocessing:
    ...
    def read_data(self):
        with open('plot_input.txt', 'r', encoding='utf-8') as f:
            self.text = f.read()
...
class NgramModel(nn.Module):
    """Smallest Input is one unit and output is one unit"""
...
if __name__ == '__main__':
    ...
    model = NgramModel(func.vocab_size)
    m = model.to(device)
    ...
    for iter in range(max_iters):
        ...
        if iter % eval_interval == 0 or iter == max_iters - 1:
            losses = func.estimate_loss()
            print(f"step {iter}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")

        # sample a batch of data
        xb, yb = func.get_batch('train')
    ...
    context = torch.tensor(func.val_data[0].view(1,1), dtype=torch.long, device=device)
    print(func.decode(m.generate(context, max_new_tokens=2000)[0].tolist()))